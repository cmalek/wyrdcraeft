...
def build_default_output_dir(input_path: Path) -> Path:
    """
    ...
    Returns:
        ``data/ocr/<input-stem>`` under repository root.

    """
    repo_root = Path(__file__).resolve().parents[3]
    return repo_root / "data" / "ocr" / input_path.stem
...
def run_old_english_ocr_pipeline(config: OldEnglishOCRConfig) -> OldEnglishOCROutput:
    ...
    ocr_pdf = _normalize_ocr_input_to_pdf(
        input_path=input_path,
...
def _resolve_output_dir(*, input_path: Path, configured: Path | None) -> Path:
    """
...
def _normalize_ocr_input_to_pdf(*, input_path: Path, workspace: Path) -> Path:
    ...
    Args:
        ...
        workspace: OCR workspace directory.

    Keyword Args:
        Uses keyword-only parameters for all inputs.

    Raises:
        RuntimeError: If input type is unsupported or image conversion fails.
...
def _is_supported_image_path(path: Path) -> bool:
    """
    Check whether path uses supported OCR image suffix.

    Args:
        path: Candidate image path.

    Returns:
        ``True`` when suffix is supported for image-to-PDF conversion.

    """
    return path.suffix.lower() in SUPPORTED_IMAGE_SUFFIXES


def _write_image_pdf(image_paths: list[Path], output_pdf: Path) -> Path:
    """
    Convert one or more image files into a PDF for olmocr ingestion.

    Args:
        image_paths: Ordered image paths to encode as PDF pages.
        output_pdf: Destination PDF path.

    Raises:
        RuntimeError: If image-to-PDF conversion fails.
    ...
    except Exception as exc:  # pragma: no cover - exact upstream error varies.
        message = f"Failed to convert OCR input images to PDF: {exc}"
...
def _require_existing_path(path: Path, *, label: str) -> None:
    """
    Ensure one required path exists.
